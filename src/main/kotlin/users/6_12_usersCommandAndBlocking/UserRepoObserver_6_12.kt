...
class UserRepoObserver_6_12 private constructor() {
    ...
    /**
     * СПИСОК ЗАДАЧ для выполнения в отдельном потоке
     */
    ...
    fun addUserToList() {
        ...
        if (age > _oldestUser.currentValue.age) {
            _oldestUser.currentValue = user // чтобы в текущем режиме отслеживать самого старшего пользователя
        }
    }
    ...
    companion object {
        ...
        private var instance: UserRepoObserver_6_12? =
            null // создание экземпляра через нуаллабельное значение переменной

        fun getInstanceUserRepository(password: String): UserRepoObserver_6_12 {
            ...
            synchronized(lock) {
                ...
                return UserRepoObserver_6_12().also { instance = it }
            }
        }
    }
}